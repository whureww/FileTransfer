...
- **扫码配对**：中继发送时自动生成二维码编码房间码，接收方扫码自动加入房间，无需手动输入
...
**UI 全面重设计（PC / 手机端视觉统一）+ 网络健壮性提升**

**PC 端界面重设计**

- 全新卡片式布局：白色圆角卡片 + 浅蓝白背景（`#F5F7FC`），与手机端视觉统一
- 靛蓝渐变主按钮（`#6B75D4 → #818CF8`），聚焦/悬停状态细化
- 自绘圆角输入框：浅米白底 + 细边框，聚焦时靛蓝高亮
- 扫码卡片重设计：左侧二维码（白边）+ 右侧房间码信息（灰色标签 + 深紫加粗房间码）
- 修复高 DPI 缩放问题：`SetProcessDpiAwarenessContext` + 布局按 DPI 动态缩放重排
- 修复窗口大小变化导致渲染异常（处理 `WM_NCPAINT` / `WM_NCACTIVATE`，`WM_SIZE` 强制重绘）
- 修复进度条文字裁剪问题
- 增大字号与控件尺寸（输入框 22px、按钮高 42px、标题栏高 50px）
- 窗口化 / 全屏均可自适应居中布局，不排版错乱

**手机端界面重设计（蓝暖杏）**

- 与 PC 端配色家族一致：靛蓝主色 + 暖杏辅色（选择文件 / 取消 / 底部标签选中态）
- 顶栏新增品牌 Logo + 版本号显示
- 分段切换：浅紫容器 + 白色选中胶囊
- 品牌输入框 / 暖杏按钮 / 圆角进度条 / 底部导航选中态全面换新
- 深浅色双主题，深色主题沿用靛蓝品牌色系

**网络健壮性**

- 局域网发现新增 `/24` 子网定向广播：兼容部分不转发全局广播的路由器 / AP 隔离环境
- 接收时保存目录不可写自动回退：exe 目录 → 系统临时目录 → 桌面
- 虚拟网卡过滤增强：扩充 VPN / 虚拟机 / 热点 / 隧道等识别关键词，并按「是否有默认网关」优先真实上网网卡
- 日志不再打印中继服务器具体地址，防止公网 IP 泄露

**其他**

- 全项目名称统一为「臻传 Silex」
- PC / 手机 / iOS / 中继服务器 / 安装器版本号统一为 `v0.1.0`
- 移除手机端硬编码的真实中继服务器地址，改为默认内置地址，防止逆向泄露

### v0.0.9 (2026-08-02)

**架构简化 + 扫码集成 + 连接修复**

- **移除独立「二维码发送」模式**：原二维码 HTTP 直连模式因防火墙/NAT 限制可靠性不足（同局域网场景仍受防火墙拦截，跨网络场景 UPnP 端口映射常失败）
...
- **PC 端防火墙规则增强**：QR 发送模式下在工作线程中二次触发防火墙规则添加（带等待确认），提高首次运行成功率
...
- 修复 `FRAME_DONE` 未校验 `received == file_size`：发送方可声明大文件、传少量数据后发 DONE，接收端误判成功。现在严格校验大小一致，否则丢弃文件并报错
- 修复 `sanitize_filename` 目录穿越漏洞：发送 `..` / `.` / 全点号文件名会写到保存目录的父目录。现在全点号文件名替换为 `received_file`，并清理文件名末尾的句点/空格
- 中继 PRNG 升级：`std::mt19937` (32-bit 状态) 升级为 `mt19937_64` (64-bit)，并用 `std::seed_seq` + 8 次 `random_device` 初始化，每次生成房间码时额外注入 `random_device` 熵，降低可预测性
- 中继线程池定期清理：每 10 秒清理已结束的工作线程（Windows 下通过 `WaitForSingleObject` 非阻塞判断），防止长时间运行线程对象内存泄漏
- 中继定期清理过期 JOIN 速率限制记录（超过 2 分钟删除），防止 IP 记录表无限增长
- 中继新增死房间兜底清理，防止极端情况内存泄漏
